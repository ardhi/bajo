...
const {
  ...
  without, uniq, isEmpty, kebabCase, isString
} = lodash
...
/**
 * Normalize file path to lock file. If the provided `file` is an absolute path, it will be returned as is.
 * Otherwise, it will be converted to a lock file path in the temporary directory of the application.
 * @param {string} file File path
 * @param {boolean} [ensureFile=true] Whether to ensure the lock file exists. Defaults to true.
 * @returns {string} Normalized lock file path.
 */
function getLockFile (file, ensureFile = true) {
  if (path.isAbsolute(file)) return file
  file = `${this.bajo.dir.data}/${kebabCase(file)}.lock`
  if (ensureFile) fs.ensureFileSync(file)
  return file
}
 ...
 *
 * @param {string} file - The path to the file to lock. If no path found, it will be created in `{tmpDir}`. Can be in many formats, see {@link https://ardhi.github.io/bajo/App.html#getPluginFile|getPluginFile} for details.
 ...
 */
async function lockFile (file, options = {}) {
  const { ensureFile = true, silent = true } = options
  const lockfile = await this.bajo.importPkg('bajo:@bybrave/proper-lockfile2')
  options.fs = fs
  try {
    file = getLockFile.call(this, file)
 ...
 * @param {string} file - The path to the file to unlock. If no path found, it will be searched in `{tmpDir}`. Can be in many formats, see {@link https://ardhi.github.io/bajo/App.html#getPluginFile|getPluginFile} for details.
 * @param {Object} [options={}] - The options for unlocking the file.
 * @returns {Promise<boolean|null>} - A promise that resolves to `true` if the file was successfully unlocked, `null` if silent and an error occurred.
 */
async function unlockFile (file, options = {}) {
 ...
 * @param {function} handler - The function to be executed at each interval
 * @param {number|string} timeout - In milliseconds or a string that can be parsed by {@link https://ardhi.github.io/aneka/global.html#parseDuration|aneka.parseDuration()}.
 * @param {Object|string} [options={}] - If a string, it will be treated as `lockFile` path. Otherwise, it will be passed to `aneka.setInterval()` as is.
 */
async function setInterval (handler, timeout, options = {}) {
  if (isString(options)) options = { lockFile: options }
  if (!path.isAbsolute(options.lockFile) && options.scope instanceof Plugin) options.lockFile = options.scope.ns + ':' + options.lockFile
  options.lockFile = getLockFile.call(this, options.lockFile)
  await aneka.setInterval(handler, timeout, options)
}
 ...
 */
const dataTypes = Object.keys(defConfig.intl.formatter)
 ...
 * @property {function} freeze Freeze object. See {@link module:Helper.freeze|freeze} for more details.
 * @property {function} findDeep Deep file file in an array of files. See {@link module:Helper.findDeep|findDeep} for more details.
 * @property {function} outmatchNs Like outmatch, but support scoped source & pattern. See {@link module:Helper.outmatchNs|outmatchNs} for more details.
 * @property {function} parseObject Parse object and normalize their values. Also support translation. See {@link module:Helper.parseObject|parseObject} for more details.
 * @property {function} lockFile Lock a file. See {@link module:Helper.lockFile|lockFile} for more details.
 * @property {function} unlockFile Unlock a file. See {@link module:Helper.unlockFile|unlockFile} for more details.
 * @property {function} setInterval Like `aneka.setInterval()`, but with Bajo's lockFile support. See {@link module:Helper.setInterval|setInterval} for more details.
 * @property {Array} dataTypes Supported data types. See {@link Bajo.TDataType|TDataType} for more details.
...
export async function ensureDirs () {
  ...
  if (!this.dir.tmp) {
    this.dir.tmp = `${this.dir.data}/tmp`
...
export async function runPlugins () {
  ...
  for (const method of methods) {
    ...
    await eachPlugins(async function () {
      ...
      if (me.app[ns].pkg.bajo.appletSupport === false && me.app.applet) {
        me.log.warn('pluginNoAppletSupport%s', ns)
        return false
      }
 ...
 *
 * @typedef {Object} TFormatterFieldMap
 * @memberof Bajo
 * @type {Object}
 ...
 * @property {string} [heading='degree']
 * @property {string} [course='degree']
 */
export const formatterFieldMap = {
  speed: 'speed',
  distance: 'distance',
  area: 'area',
  degree: 'degree',
...
/**
 * @typedef {Object} TFormatter
 * @memberof Bajo
 * @type {Object}
 * @property {Object} metric - Metric formatter
 * @property {Object} metric.speedFn - Function to format speed in metric
 * @property {string} metric.speedUnit - Speed unit in metric
 * @property {Object} metric.distanceFn - Function to format distance in metric
 * @property {string} metric.distanceUnit - Distance unit in metric
 * @property {Object} metric.areaFn - Function to format area in metric
 * @property {string} metric.areaUnit - Area unit in metric
 * @property {Object} metric.degreeFn - Function to format degree in metric
 * @property {string} metric.degreeUnit - Degree unit in metric
 * @property {string} metric.degreeUnitSep - Degree unit separator in metric
 * @property {Object} imperial - Imperial formatter
 * @property {Object} imperial.speedFn - Function to format speed in imperial
 * @property {string} imperial.speedUnit - Speed unit in imperial
 * @property {Object} imperial.distanceFn - Function to format distance in imperial
 * @property {string} imperial.distanceUnit - Distance unit in imperial
 * @property {Object} imperial.areaFn - Function to format area in imperial
 * @property {string} imperial.areaUnit - Area unit in imperial
 * @property {Object} imperial.degreeFn - Function to format degree in imperial
 * @property {string} imperial.degreeUnit - Degree unit in imperial
 * @property {string} imperial.degreeUnitSep - Degree unit separator in imperial
 * @property {Object} nautical - Nautical formatter
 * @property {Object} nautical.speedFn - Function to format speed in nautical
 * @property {string} nautical.speedUnit - Speed unit in nautical
 * @property {Object} nautical.distanceFn - Function to format distance in nautical
 * @property {string} nautical.distanceUnit - Distance unit in nautical
 * @property {Object} nautical.areaFn - Function to format area in nautical
 * @property {string} nautical.areaUnit - Area unit in nautical
 * @property {Object} nautical.degreeFn - Function to format degree in nautical
 * @property {string} nautical.degreeUnit - Degree unit in nautical
 * @property {string} nautical.degreeUnitSep - Degree unit separator in nautical
 */
