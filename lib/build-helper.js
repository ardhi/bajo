...
function stackInfo (name, ...args) {
  if (this.bajo.config.env === 'PROD') return
  if (this.bajo.config.log.details.includes('helper')) {
    const stack = (new Error().stack.split("at ")[3]).trim()
    const line = stack.match(/\(([^()]*)\)/)[1]
    this.bajo.log.trace({ args, line }, `Call helper: %s()`, name)
  } else this.bajo.log.trace(`Call helper: %s()`, name)
}
